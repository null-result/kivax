...
- [ ] If the CLI or the flow changed, the affected agents and skills under
      `src/kivax/data/` were updated too — they describe the CLI to the assistant,
      and drift there is invisible until someone hits it in a real session
- [ ] If anything moved under `src/kivax/data/`, the `package` CI job still passes —
      it is the only check that sees the built wheel, where a wrong `package-data`
      glob is invisible from a checkout
