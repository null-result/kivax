...
5. If the active feature is past `plan`: `kivax lessons check` (informational here — it's the audit that enforces it). An unacknowledged lesson reported now is a one-line fix in `plan.md`; reported at the audit gate it's a blocked merge.
6. Summarize for the human: the active feature and its phase, its REQs by status (pending/red/green/invalidated), **any work left in flight and which agent owns it**, traceability coverage, wiki freshness (if it exists), unacknowledged lessons (if any), and the next recommended skill to run.
7. **Call out any NON-active feature with stale hashes explicitly.** That means an older feature's spec has drifted from its tests, it blocks the audit, and it needs `kivax feature switch <NN>` followed by the `kivax-evolve` skill. It's the finding most easily lost in a status report, and the one the human most needs.
