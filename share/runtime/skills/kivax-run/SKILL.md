...
Runs the flow from the current phase (`kivax state show`), following the `pipeline` list in `.kivax/config.yml` (`kivax state next` gives each following phase — this includes user-added custom phases), chaining automatically until the first of:
...
4. Never chain into the merge: it isn't a flow phase and is always human. (A deploy IS chainable if — and only if — the user added it as a pipeline phase with its own skill; then its gate decides.)
