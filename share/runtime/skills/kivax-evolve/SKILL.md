...
7. `evolve` gates `human` — invalidating tests is destructive, so this one always stops. Present the impact (affected REQs, invalidated tests, reingested wiki pages) and wait for approval, then relaunch the `kivax-tdd` skill for the affected IDs only, chaining the rest of the flow per their gates.
