...
3. Check `kivax state gate audit`. With `human` (recommended), present the auditor's verdict + the reviewer's report + any constitution violations and wait for approval. With `auto`, only proceed if the auditor gave PASSING, the reviewer left NO BLOCKING findings, and there are NO constitution violations — any of these is an exception and stops the flow regardless, to be routed (uncovered spec → the `kivax-evolve` skill; code → implementer via a mini TDD cycle for the affected REQ, through the `kivax-tdd` skill; constitution violation → the human decides whether to fix the change or amend the constitution).
...
6. Constitution check (only if `CONSTITUTION.md` exists, `paths.constitution` in config): read it and cross-check the actual diff against its stated principles — a judgment call, not something `kivax trace` computes. Any conflict is a `CONSTITUTION-VIOLATION:`, reported separately from the PASSING/NOT PASSING verdict (a change can be perfectly traceable and still violate a principle). This is the last-chance check — `kivax-plan` already checked the intended plan, this checks what was actually built.
7. Only if the traceability verdict is PASSING: `kivax trace --update-lock` (it rewrites the lock for EVERY feature, and refuses rather than drop entries it can't account for) and update the state with `kivax state`. A constitution violation does NOT block the lock update by itself (traceability and constitutional compliance are independent concerns) — but it DOES stop the flow at the gate below regardless of `auto`/`human`.

### Hard rules
- Never "approve with observations": the traceability result is PASSING or NOT PASSING with an exact list of violations.
- You don't fix anything yourself: every violation goes back with the suggested responsible role (missing coverage → test-writer; stale hash → the `kivax-evolve` flow; orphan → compiler/analyst; unacknowledged lesson → tech-planner; constitution violation → the human).
...
- Never resolve a `CONSTITUTION-VIOLATION:` yourself (by deciding it's fine, or by amending the constitution) — it always goes to the human.
...
2. Review in this priority order:
   ...
   - **Lessons honored**: read `plan.md`'s `## Lessons applied` and check the diff actually does what each line claims. The auditor only verified the lines exist; whether the code lives up to them is yours to judge, and a plan that promises to honor a lesson and doesn't is BLOCKING.
