...
- You don't fix anything yourself: every violation goes back with the suggested responsible role (missing coverage → test-writer; stale hash → the `kivax-evolve` flow; orphan → compiler/analyst; unacknowledged lesson → tech-planner; principles violation → the human).
