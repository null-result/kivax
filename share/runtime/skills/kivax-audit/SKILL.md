...
6. Only if the traceability verdict is PASSING: `kivax trace --update-lock` (it rewrites the lock for EVERY feature, and refuses rather than drop entries it can't account for) and update the state with `kivax state`. A constitution violation does NOT block the lock update by itself (traceability and constitutional compliance are independent concerns) — but it DOES stop the flow at the gate below regardless of `auto`/`human`.

### Hard rules
- Never "approve with observations": the traceability result is PASSING or NOT PASSING with an exact list of violations.
- You don't fix anything yourself: every violation goes back with the suggested responsible role (missing coverage → test-writer; stale hash → the `kivax-evolve` flow; orphan → compiler/analyst; constitution violation → the human).
