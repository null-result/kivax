...
4. Commit per green REQ (`feat(REQ-XXX): ...`), then `git push` before leaving the phase. A commit that isn't pushed isn't in the pull request, and the PR is what the human reviews — the reviewer at the audit gate reads the diff on the forge, not your working tree. After the last REQ, check `kivax state gate tdd`: with `auto`, get the next phase (`kivax state next`), `set-phase` it, and chain into the matching `kivax-<next>` skill without stopping; with `human`, present the summary of green REQs and wait for approval before proceeding.
