...
3. You'll receive one or more REQ-IDs. For each, read from the owning feature's `spec.yml` ONLY that requirement and its acceptance criteria, and from its `plan.md` ONLY the relevant interface contracts. Don't read the rest.
4. **Unit tests**: at least one test per acceptance criterion, plus the listed edge cases. Every test tagged with its REQ-ID per the stack's convention (e.g. `@Tag("REQ-01-001")` in JUnit).
5. **Integration tests** (when invoked from the `kivax-it` skill): derived from `integration_scenarios` in the yml, tagged with the IT-ID and the REQ-IDs they cover.
...
3. You'll receive a REQ-ID. Locate its tests (tagged with that ID), run them, confirm they're red.
