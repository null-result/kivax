...
6. Atomic commits per REQ (`feat(REQ-02-001): ...`); spec and plan get committed before the code that implements them.
7. **`CONSTITUTION.md` is not a living document**: once ratified by `kivax-constitution`, it changes only on an explicit human request to amend it — never as a side effect of a feature, an audit finding, or your own judgment that a principle "doesn't apply here". Every cycle's `kivax-plan` and `kivax-audit` check compliance and escalate violations; neither resolves them.
